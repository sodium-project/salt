...
#include <salt/foundation/iterator_adapter.hpp>
#include <salt/foundation/uninitialized_construct.hpp>
...
public:
    ...
    constexpr static_vector(InputIterator first, InputIterator last) noexcept : static_vector() {
        check_free_space(std::distance(first, last));
        auto d_begin = begin();
...
private:
    constexpr void check_free_space(meta::integral auto new_size) const noexcept {
        SALT_ASSERT(static_cast<size_type>(new_size) <= capacity());
    }
    ...
    constexpr iterator move_elements(const_iterator position, meta::integral auto n) noexcept {
        check_free_space(size() + static_cast<size_type>(n));
