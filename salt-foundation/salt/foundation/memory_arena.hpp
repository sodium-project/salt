...
#include <salt/foundation/allocator_traits.hpp>
#include <salt/foundation/construct_at.hpp>
#include <salt/foundation/default_allocator.hpp>
#include <salt/foundation/memory_block.hpp>
...
private:
    constexpr auto info() noexcept {
        return allocator_info{"salt::fdn::growing_block_allocator", this};
    }
...
public:
    ...
    constexpr memory_block allocate_block() noexcept {
        SALT_ASSERT(block_size_, "salt::fdn::fixed_block_allocator ran out of memory.");
        auto         memory = allocator_traits::allocate_array(allocator(), block_size_, 1,
                                                               detail::max_alignment);
        memory_block block(memory, block_size_);
        block_size_ = 0u;
...
private:
    constexpr auto info() noexcept {
        return allocator_info{"salt::fdn::fixed_block_allocator", this};
    }
