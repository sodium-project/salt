...
constexpr void print_define(fast_io::io_reserve_type_t<char, allocator_info>,
    ...
    io::print(stream, "Allocator ", info.name, " (at ", mnp::pointervw(info.allocator), ") ");
}
...
void default_leak_handler(allocator_info const& info, std::ptrdiff_t amount) noexcept {
    if (amount > 0)
        log::error(info, "leaked ", amount, " bytes");
    else
        log::error(info, "has deallocated ", amount, " bytes more than ever allocated");
}
...
void default_invalid_ptr_handler(allocator_info const& info, const void* ptr) noexcept {
    ...
    log::error("Deallocation function of ", info, "received invalid pointer ", pointervw(ptr));
}
...
void default_buffer_overflow_handler(const void* memory, std::size_t node_size,
    ...
    using namespace fast_io::mnp;
    log::error("Buffer overflow at address ", pointervw(ptr),
               " detected, corresponding memory block", pointervw(memory), " has only size ",
               node_size);
}
