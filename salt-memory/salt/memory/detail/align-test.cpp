...
TEST_CASE("salt::detail::align_offset", "[salt-memory/align.hpp]") {
...
TEST_CASE("salt::detail::is_aligned", "[salt-memory/align.hpp]") {
    auto ptr = reinterpret_cast<void*>(0);
...
TEST_CASE("salt::detail::alignment_for", "[salt-memory/align.hpp]") {
...
TEST_CASE("salt::detail::ilog2", "[salt-memory/align.hpp]") {
...
TEST_CASE("salt::detail::ilog2_ceil", "[salt-memory/align.hpp]") {
    SECTION("Check everything up to 2^16") {
        for (std::size_t i = 0; i != 16; ++i) {
            auto power = 1u << i;
