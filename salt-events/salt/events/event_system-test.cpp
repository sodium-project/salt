...
TEST_CASE("salt::Event_bus", "[salt-events/event_system.hpp]") {
    ...
    SECTION("test attach") {
        Event_bus<Event<A>, Event<B>> bus;
        int                           a     = 0;
        int                           b     = 0;
        int                           count = 0;
        std::string                   s;

        bus.attach<Event<A>>([&](auto& e) {
            count++;
            a = e->a;
            b = e->b;
            e.consume();
        ...
        bus.attach<Event<A>>([&](auto&) {
            count++;
            REQUIRE_FALSE(true); // always false checking if event consuming
        });

        bus.attach<Event<B>>([&](auto& e) {
            count++;
            s = e->s;
            REQUIRE(s == "B");
        });
    ...
    SECTION("test detach") {
        ...
        auto callback_id = bus.attach<Event<A>>([&](auto& e) {
            count++;
            a = e->a;
            b = e->b;
            REQUIRE(a == 1);
        ...
        bus.detach<Event<A>>(callback_id);
        bus.dispatch<Event<A>>(1, 2);
        REQUIRE(count == 1);
    }
}
